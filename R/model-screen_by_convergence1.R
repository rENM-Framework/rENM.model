...
screen_by_convergence1 <- function(alpha_code,
  ...
  if (is.null(seed)) {
    ...
    message(sprintf("[screen_by_convergence1] Using randomly chosen seed = %d", seed))
  } else {
    if (!is.numeric(seed) || length(seed) != 1L || !is.finite(seed)) {
      stop("`seed` must be NULL or a finite numeric scalar.", call. = FALSE)
    }
    seed <- as.integer(seed)
    message(sprintf("[screen_by_convergence1] Using user-supplied seed = %d", seed))
  }
  ...
  ## --- background (reused) --------------------------------------------------
  background <- randomPoints(env_stack, background_n)
  ...
  if (is.null(batch_samples)) {
    ...
    message(sprintf("[screen_by_convergence1] using fallback batch_samples = %d (p = %d%s)",
                    batch_samples, p, if (fast) ", fast" else ""))
  } else {
    batch_samples <- as.integer(batch_samples)
    if (!is.finite(batch_samples) || batch_samples < 1L) {
      batch_samples <- compute_fallback(p, fast)
      message(sprintf("[screen_by_convergence1] provided batch_samples invalid; using fallback = %d", batch_samples))
    }
  }
  ...
  # Balanced pairing sampler: each var appears ~appearances times with diverse partners
  balanced_pairs <- function(vars, appearances) {
    appearances <- max(1L, as.integer(appearances))
    v <- rep(vars, each = appearances)
    v <- sample(v)
    n <- length(v) - (length(v) %% 2)
    if (n < 2) return(list())
    as.list(split(v[1:n], rep(1:(n / 2), each = 2)))
  }
  ...
  for (iter in seq_len(max_iter)) {
    ...
    batch_res <- foreach(
      ...
      model <- try(
        dismo::maxent(sub_env, occ, a = background, args = me_args),
        silent = TRUE
      )
      if (inherits(model, "try-error")) {
        return(data.frame(var = p, perm_imp = NA_real_))
      }

      res <- model@results
      keep <- grepl("permutation.importance", rownames(res))
      perm_imp <- suppressWarnings(as.numeric(res[keep, 1]))
      if (length(perm_imp) != length(p)) {
        perm_imp <- rep_len(perm_imp, length.out = length(p))
      }
  ...
  message("[screen_by_convergence1] Writing updated files:")
